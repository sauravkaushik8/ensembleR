...
#'
#'@param myControl Basic parameter tuning as in 'caret package'. By default, it uses  CV two times with single repeat.
#'
...
ensemble<-function(training,testing,outcomeName,BaseModels,TopModel,myControl = caret::trainControl(method='cv',number=2,repeats=1,returnResamp='none'))
{
  ...
  for(i in 1:length(BaseModels))
  {
    ...
    model <- caret::train(training[,predictors], training[,outcomeName], method=BaseModels[i], trControl=myControl)
